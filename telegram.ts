...
function markdownToTelegramHtml(md: string): string {
  ...
  // Markdown formatting
  workingText = workingText.replace(/\*\*([^*]+)\*\*/g, "<b>$1</b>");
  workingText = workingText.replace(/(^|[^*])\*([^*]+)\*/g, "$1<i>$2</i>");
  workingText = workingText.replace(/~~([^~]+)~~/g, "<s>$1</s>");

  // Headers: #, ##, ###
  workingText = workingText.replace(/^### (.*)$/gm, "<b>$1</b>");
  workingText = workingText.replace(/^## (.*)$/gm, "<b>$1</b>");
  workingText = workingText.replace(/^# (.*)$/gm, "<b>$1</b>");
  ...
  // Markdown Table fallback for Telegram (Telegram HTML does not support <table>):
  // Format tables cleanly as monospace preformatted blocks
  ...
  inlineCodes.forEach((code, idx) => {
    html = html.replace(`@@TG_INLINE_CODE_${idx}@@`, code);
  });
  codeBlocks.forEach((block, idx) => {
    html = html.replace(`@@TG_CODE_BLOCK_${idx}@@`, block);
  });
