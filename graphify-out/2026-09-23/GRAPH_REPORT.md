...
## Corpus Check
- 21 files · ~72,457 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 170 nodes · 331 edges · 10 communities (7 shown, 3 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 15 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `34d87786`
...
2. `TelegramApiClient` - 21 edges
3. `MatrixProgressReporter` - 12 edges
4. `MatrixQueue` - 12 edges
5. `TelegramQueue` - 11 edges
6. `handleMatrixCommand()` - 9 edges
7. `MatrixConfig` - 9 edges
8. `handleSlashCommand()` - 9 edges
...
Cohesion: 0.18
Nodes (13): BoundedEventCache, DEFAULT_CONFIG, formatFileSize(), getHomeDir(), getMediaDir(), getSyncTokenPath(), loadConfig(), loadSavedSyncToken() (+5 more)
...
Cohesion: 0.14
Nodes (17): getQuickActionMarkup(), handleSlashCommand(), DEFAULT_CONFIG, formatFileSize(), getHomeDir(), getMediaDir(), loadConfig(), cleanAssistantText() (+9 more)
...
- **Why does `MatrixQueue` connect `MatrixQueue` to `index.ts`?**
